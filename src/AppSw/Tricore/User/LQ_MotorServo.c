...
sint32 NowTime = 0;
uint16 BatVolt = 0;
volatile float lowspeed=0;// 电池电压采集
extern pid_param_t pids,pidspeed;
extern float Errorspeed;//pid定义
//电机频率
...
void Servo_Set(void)
{
    Re_servo=PidLocCtrl(&pids, Error);
    if(Re_servo<-2000)
    {
        Re_servo=-1000;
    }

    servoTurnDuty=(int)Re_servo;
    Servo_duty=Servo_Center_Mid+servoTurnDuty;
    ServoCtrl(Servo_duty);
}
void speedset(void)
{
    MotorCtrl((int)(constrain_float( PidIncCtrl(&pidspeed, Errorspeed), lowspeed, 5000.0)));
